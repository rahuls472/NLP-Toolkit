...
class NLPApp:
    ...
    def perform_NER(self):
        ...
        btn5 = ttk.Button(self.root, text="Generate", command=self.ner_result, bootstyle=PRIMARY, width=40)
        btn5.pack(pady=(10))
    ...
    def ner_result(self):
        entity = self.search_Entity.get()
        para = self.para_input.get()
        MAX_ENTITY_LENGTH = 200

        # Check the length of the searched entity
        if len(entity) > MAX_ENTITY_LENGTH:  # Replace MAX_ENTITY_LENGTH with the actual limit
            messagebox.showerror('Error', f'The searched entity is too long. Maximum length is {MAX_ENTITY_LENGTH} characters.')
            return

        result = self.obj1.Ner_analysis(entity, para)
